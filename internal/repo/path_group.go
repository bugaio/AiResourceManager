...
func (r *PathGroupRepo) InsertPathGroup(g *model.PathGroup) error {
	...
	_, err := r.db.Conn.Exec(
		`INSERT INTO path_group (id, name, skill_path, agent_path, config_path, prompt_path, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		g.ID, g.Name, g.SkillPath, g.AgentPath, g.ConfigPath, g.PromptPath, g.CreatedAt, g.UpdatedAt,
	)
	if err != nil {
...
func (r *PathGroupRepo) ListPathGroups() ([]model.PathGroup, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&g.ID, &g.Name, &g.SkillPath, &g.AgentPath, &g.ConfigPath, &g.PromptPath, &g.CreatedAt, &g.UpdatedAt); err != nil {
